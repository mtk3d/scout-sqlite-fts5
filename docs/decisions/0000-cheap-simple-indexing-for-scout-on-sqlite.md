...
The decision recorded here — cheap to adopt, cheap to operate — is the part meant to hold. How it is served is not settled: version 1 may write and store documents in a completely different way, and these records will be superseded as that happens. Version 0 will not do that to an application mid-flight — the format stays put across `0.x` — and what keeps the eventual change cheap is [decision 2](0002-tables-derived-from-models.md): the index is derived from models rather than owned as a schema, so a change of layout costs a rebuild rather than a migration.
